...
// Proxies audio upload to Roblox. Cookie used only for this request, never stored.
...
export default async function handler(req, res) {
  ...
  } catch (e) { console.error('csrf:', e.message); }

  // ── Parse audio + metadata from form ──────────────────────────────────────
  ...
  uploadForm.append('fileContent', new Blob([audioBuffer], { type: 'audio/mpeg' }), fileName);
  ...
  try {
    ...
    console.log('roblox upload:', r.status, text.slice(0, 300));
    if (!r.ok) return res.status(502).json({ error: `roblox upload error (${r.status}): ${text}` });
  ...
  if (!assetId) return res.status(504).json({ error: 'timed out — roblox took too long' });
